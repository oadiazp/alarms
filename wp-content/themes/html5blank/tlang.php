...
class tlang{
	...
	public function get_language($feature = ""){
		...
		if ($feature == 'data')
			...
			return (!empty($user_languages)) ? $user_languages[0][1] : "en";
	}
	...
	public function changeLang($new_lang, $opt = 0, $node = ''){
		if ($_SESSION["current_lang"] != $new_lang || $opt == 1){
			if (!$opt) $_SESSION["current_lang"] = $new_lang;
			$this->readXMLLang($new_lang, 0, $node);
		}
		else{
	...
	public function readXMLLang($lang, $mode = 0, $node = ''){
		$array = array(); $x = 0;
		if ($lang != "en" && $lang != "de")
			$lang = $_SESSION["current_lang"] = "en";
		$path = (file_exists("XML/tlang-$lang.xml")) ? "XML/tlang-$lang.xml" : "XML/tlang-en.xml";
		//$path = "XML/tlang-de.xml";
		$xml = simplexml_load_file($path);
		if ($node){
			foreach($xml->xpath("//$node") as $child){
				foreach($child as $key => $value){
					$array[$x]["key"] = $_SESSION["tlang"][$x]["key"] = $key;
					$array[$x]["value"] = $_SESSION["tlang"][$x]["value"] = (string) $value;
					$x++;
				}
			}
		}
	...
	public function saveXML($lang, $node, $vals){
		$array = array(); $x = 0;
		$path = (file_exists("XML/tlang-$lang.xml")) ? "XML/tlang-$lang.xml" : "XML/tlang-en.xml";
		//$path = "XML/tlang-de.xml";
		$xml = simplexml_load_file($path);
			...
			if (isset($_POST['fn'])){
				switch($_POST['fn']){
					...
					case "changeLang":
						...
						$this->changeLang($_POST['lang'], $opt, $node);
					break;
					case "saveXML":
						$this->saveXML($_POST['lang'], $_POST['node'], $_POST['vals']);
					break;
				}
			}
		}
